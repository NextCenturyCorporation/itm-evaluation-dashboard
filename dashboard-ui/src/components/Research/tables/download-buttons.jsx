    ...
    return (
        <div className={"option-section " + (filteredData.length < formattedData.length ? "adjusted-margin" : "")}>
            {filteredData.length < formattedData.length || Object.keys(filteredData[0] ?? {}).length < Object.keys(formattedData[0] ?? {}).length ? <div className="downloadGroup">
                <button className='downloadBtn' onClick={() => exportToExcel(fileName, filteredFormattedData, HEADERS, isParticipantData)}>Download All Data</button>
                <button className='downloadBtn' onClick={() => exportToExcel(fileName + ' (filtered)', filteredFilteredData, HEADERS, isParticipantData)}>Download Filtered Data</button>
            </div> :
                <button className='downloadBtn' onClick={() => exportToExcel(fileName, filteredFormattedData, HEADERS, isParticipantData)}>Download All Data</button>
            }
