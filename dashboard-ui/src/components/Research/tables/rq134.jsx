...
const GET_SURVEY_RESULTS_BY_EVAL = gql`
...
export function RQ134({ evalNum, tableTitle }) {
    ...
    const { loading: loadingSurveyByEval, error: errorSurveyByEval, data: dataSurveyByEval } = useQuery(GET_SURVEY_RESULTS_BY_EVAL, { variables: { evalNumber: evalNum } });
    const { loading: loadingTextByEval, error: errorTextByEval, data: dataTextByEval } = useQuery(GET_TEXT_RESULTS_BY_EVAL, { variables: { evalNumber: evalNum } });
    ...
    const { data: dreAdms } = useQuery(GET_ADM_DATA, { variables: { "evalNumber": 4 } });
    const { data: juneAdms } = useQuery(GET_ADM_DATA, { variables: { "evalNumber": 8 } });
    const { data: julyAdms } = useQuery(GET_ADM_DATA, { variables: { "evalNumber": 9 } });
    ...
    const { data: dreSurveyByEval } = useQuery(GET_SURVEY_RESULTS_BY_EVAL, { variables: { evalNumber: 4 } });
    const { data: janSurveyByEval } = useQuery(GET_SURVEY_RESULTS_BY_EVAL, { variables: { evalNumber: 6 } });
    const { data: juneSurveyByEval } = useQuery(GET_SURVEY_RESULTS_BY_EVAL, { variables: { evalNumber: 8 } });
    const { data: julySurveyByEval } = useQuery(GET_SURVEY_RESULTS_BY_EVAL, { variables: { evalNumber: 9 } });
    ...
    const { data: juneTextByEval } = useQuery(GET_TEXT_RESULTS_BY_EVAL, { variables: { evalNumber: 8 } });
    const { data: julyTextByEval } = useQuery(GET_TEXT_RESULTS_BY_EVAL, { variables: { evalNumber: 9 } });
    ...
    const currentSurveyWrapper = React.useMemo(() => (
        dataSurveyByEval?.getAllSurveyResultsByEval ? { getAllSurveyResults: dataSurveyByEval.getAllSurveyResultsByEval } : undefined
    ), [dataSurveyByEval]);
    const currentTextWrapper = React.useMemo(() => (
        dataTextByEval?.getAllScenarioResultsByEval ? { getAllScenarioResults: dataTextByEval.getAllScenarioResultsByEval } : undefined
    ), [dataTextByEval]);

    const dreSurveyWrapper = React.useMemo(() => (
        dreSurveyByEval?.getAllSurveyResultsByEval ? { getAllSurveyResults: dreSurveyByEval.getAllSurveyResultsByEval } : undefined
    ), [dreSurveyByEval]);
    const janSurveyWrapper = React.useMemo(() => (
        janSurveyByEval?.getAllSurveyResultsByEval ? { getAllSurveyResults: janSurveyByEval.getAllSurveyResultsByEval } : undefined
    ), [janSurveyByEval]);
    const juneSurveyWrapper = React.useMemo(() => (
        juneSurveyByEval?.getAllSurveyResultsByEval ? { getAllSurveyResults: juneSurveyByEval.getAllSurveyResultsByEval } : undefined
    ), [juneSurveyByEval]);
    const julySurveyWrapper = React.useMemo(() => (
        julySurveyByEval?.getAllSurveyResultsByEval ? { getAllSurveyResults: julySurveyByEval.getAllSurveyResultsByEval } : undefined
    ), [julySurveyByEval]);

    const dreTextWrapper = React.useMemo(() => (
        dreTextByEval?.getAllScenarioResultsByEval ? { getAllScenarioResults: dreTextByEval.getAllScenarioResultsByEval } : undefined
    ), [dreTextByEval]);
    const janTextWrapper = React.useMemo(() => (
        janTextByEval?.getAllScenarioResultsByEval ? { getAllScenarioResults: janTextByEval.getAllScenarioResultsByEval } : undefined
    ), [janTextByEval]);
    const juneTextWrapper = React.useMemo(() => (
        juneTextByEval?.getAllScenarioResultsByEval ? { getAllScenarioResults: juneTextByEval.getAllScenarioResultsByEval } : undefined
    ), [juneTextByEval]);
    const julyTextWrapper = React.useMemo(() => (
        julyTextByEval?.getAllScenarioResultsByEval ? { getAllScenarioResults: julyTextByEval.getAllScenarioResultsByEval } : undefined
    ), [julyTextByEval]);

    const surveyWrapperFor = (e) => {
        if (e === 4) return dreSurveyWrapper ?? currentSurveyWrapper;
        if (e === 6) return janSurveyWrapper ?? currentSurveyWrapper;
        if (e === 8) return juneSurveyWrapper ?? currentSurveyWrapper;
        if (e === 9) return julySurveyWrapper ?? currentSurveyWrapper;
        return currentSurveyWrapper;
    };
    const textWrapperFor = (e) => {
        if (e === 4) return dreTextWrapper ?? currentTextWrapper;
        if (e === 6) return janTextWrapper ?? currentTextWrapper;
        if (e === 8) return juneTextWrapper ?? currentTextWrapper;
        if (e === 9) return julyTextWrapper ?? currentTextWrapper;
        return currentTextWrapper;
    };

    React.useEffect(() => {
        if (currentSurveyWrapper?.getAllSurveyResults && dataParticipantLog?.getParticipantLog && currentTextWrapper?.getAllScenarioResults &&
            ...
            const data = getRQ134Data(evalNum, currentSurveyWrapper, dataParticipantLog, currentTextWrapper, dataADMs, comparisonData, dataSim);
    ...
    }, [dataParticipantLog, currentSurveyWrapper, currentTextWrapper, dataADMs, comparisonData, evalNum, includeDRE, includeJAN, includeJune, includeJuly, includeUSEvals, dreAdms, juneAdms, julyAdms, dreSim, janSim, juneSim, julySim, dataSim]);

    const includeExtraData = (data, evalToAdd, simData, admsToUse) => {
        const surveyW = surveyWrapperFor(evalToAdd);
        const textW = textWrapperFor(evalToAdd);

        const addedData = getRQ134Data(evalToAdd, surveyW, dataParticipantLog, textW, admsToUse, comparisonData, simData, evalToAdd == 4);
        if (evalToAdd == 6) {
    ...
    if (loadingParticipantLog || loadingSurveyByEval || loadingTextByEval || loadingADMs || loadingComparisonData || loadingSim) return <p>Loading...</p>;
    if (errorParticipantLog || errorSurveyByEval || errorTextByEval || errorADMs || errorComparisonData || errorSim) return <p>Error :</p>;
