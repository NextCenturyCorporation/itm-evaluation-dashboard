...
/* CHANGED: eval-scoped text results */
...
export function RQ5_PH1({ evalNum }) {
    ...
    /* CHANGED: eval-scoped text; no client-side filter by eval needed */
    const { loading: loadingTextResults, error: errorTextResults, data: dataTextResults } = useQuery(GET_TEXT_RESULTS_BY_EVAL, {
    ...
    React.useEffect(() => {
        if (dataParticipantLog?.getParticipantLog && dataTextResults?.getAllScenarioResultsByEval && comparisonData?.getHumanToADMComparison && comparisonData?.getADMTextProbeMatches) {
            const allObjs = [];
            const allTA1s = [];
            const allScenarios = [];
            const allAttributes = [];
            ...
            /* CHANGED: already eval-filtered */
            const textResults = dataTextResults.getAllScenarioResultsByEval;
