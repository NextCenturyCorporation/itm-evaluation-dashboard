    ...
    switch (scenario) {
        ...
        case 'DryRunEval-IO4-eval':
            if (Number(surveyVersion) === 4) {
                ...
                target = ioTargets.find((t) => String(t.target) === 'ADEPT-DryRun-Ingroup Bias-1.0').target;
...
export function getTextScenariosForParticipant(pid, participantLog) {
    ...
    const matchedLog = participantLog.getParticipantLog.find(
        log => String(log['ParticipantID']) === String(pid)
    );
