...
const generateServerTimestamp = () => {
  ...
  return date
    .toString()
    .replace(
      /GMT-0[45]00 \(Eastern (Daylight|Standard) Time\)/,
      isDST
  ...
  Query: {
    checkUserExists: async (obj, args, context, infow) => {
      const userByEmail = await context.db.collection("users").findOne({
        $or: [
          {
            "emails.address": {
              $regex: `^${args.email.toLowerCase().trim()}$`,
              $options: "i"
            }
          },
          {
            username: {
              $regex: `^${args.username.toLowerCase().trim()}$`,
              $options: "i"
            }
          }
        ]
    ...
    getEvalIdsForAllHistory: async (obj, args, context, inflow) => {
      return await context.db
        ...
        .aggregate([
          {
            $group: {
              _id: { evalNumber: "$evalNumber", evalName: "$evalName" }
            }
          }
        ])
        .sort({ evalNumber: -1 })
    ...
    getAllHistoryByID: async (obj, args, context, inflow) => {
      return await context.db
        .collection("admTargetRuns")
        .find(
          { "history.response.id": args.historyId },
          {
            projection: {
              evaluation: 1,
              "history.parameters.adm_name": 1,
              "history.parameters.ADM Name": 1,
              "history.response.score": 1,
              evalNumber: 1
            }
          }
    ...
    getScenarioNames: async (obj, args, context, inflow) => {
      return await context.db
        .collection("scenarios")
        .aggregate([{ $group: { _id: { id: "$id", name: "$name" } } }])
        .toArray()
        .then((result) => {
          return result;
        });
    ...
    getScenarioNamesByEval: async (obj, args, context, inflow) => {
      ...
      for (const scenario of scenarios) {
        const hasData = await context.db.collection("admTargetRuns").findOne({
    ...
    getAlignmentTargetsPerScenario: async (obj, args, context, inflow) => {
      ...
      let alignmentTargets = await context.db
        .collection("admTargetRuns")
    ...
    getTestByADMandScenario: async (obj, args, context, inflow) => {
      ...
      if (
        args["alignmentTarget"] == null ||
        args["alignmentTarget"] == undefined ||
        args["alignmentTarget"] == "null"
      ) {
        queryObj = {
          $and: [{ "history.response.id": args["scenarioID"] }]
        };
    ...
    getAllSurveyResults: async (obj, args, context, inflow) => {
      ...
      const excludeTestID = {
        ...
        "results.Participant ID Page.questions.Participant ID.response": {
          $not: /test/i
        },
        "Participant ID.questions.Participant ID.response": { $not: /test/i },
        "Participant ID Page.questions.Participant ID.response": {
          $not: /test/i
        }
      };
    ...
    getAllTextScenariosDRE: async (obj, args, context, inflow) => {
      return await context.db
        .collection("userScenarioResults")
        .distinct("scenario_id", { evalNumber: 4 })
        .then((result) => {
          return result;
        });
    },
    ...
    getUsers: async (obj, args, context, infow) => {
      ...
      if (session?.valid && session?.userId == user?._id && user?.admin) {
        ...
        throw new GraphQLError(
          "Users outside of the admin group cannot access user data.",
          {
            extensions: { code: "404" }
          }
        );
    ...
    countHumanGroupFirst: async (obj, args, context, info) => {
      return await context.db
        .collection("surveyResults")
        .countDocuments({ "results.humanGroupFirst": true });
    },
    countAIGroupFirst: async (obj, args, context, info) => {
      return await context.db
        .collection("surveyResults")
        .countDocuments({ "results.aiGroupFirst": true });
    },
    getParticipantLog: async (obj, args, context, info) => {
      return await context.db
        .collection("participantLog")
        .find()
        .toArray()
        .then((result) => {
          return result;
    ...
    getADMTextProbeMatches: async (obj, args, context, info) => {
      return await context.db
        .collection("admVsTextProbeMatches")
        .find()
        .toArray()
        .then((result) => {
          return result;
    ...
    getShowDemographics: async (obj, args, context, info) => {
      const demographics = await context.db
        .collection("surveyVersion")
        .findOne();
  ...
  Mutation: {
    updateAdminUser: async (obj, args, context, inflow) => {
      ...
      if (session?.valid && session?.userId == user?._id && user?.admin) {
        ...
        throw new GraphQLError(
          "Users outside of the admin group cannot update administrator status.",
          {
            extensions: { code: "404" }
          }
        );
    ...
    updateEvaluatorUser: async (obj, args, context, inflow) => {
      ...
      if (session?.valid && session?.userId == user?._id && user?.admin) {
        ...
        throw new GraphQLError(
          "Users outside of the admin group cannot update evaluator status.",
          {
            extensions: { code: "404" }
          }
        );
    ...
    updateExperimenterUser: async (obj, args, context, inflow) => {
      ...
      if (session?.valid && session?.userId == user?._id && user?.admin) {
        ...
        throw new GraphQLError(
          "Users outside of the admin group cannot update experimenter status.",
          {
            extensions: { code: "404" }
          }
        );
    ...
    updateAdeptUser: async (obj, args, context, inflow) => {
      ...
      if (session?.valid && session?.userId == user?._id && user?.admin) {
        ...
        throw new GraphQLError(
          "Users outside of the admin group cannot update ADEPT user status.",
          {
            extensions: { code: "404" }
          }
        );
    ...
    updateUserApproval: async (obj, args, context, inflow) => {
      ...
      if (session?.valid && session?.userId == user?._id && user?.admin) {
        ...
        throw new GraphQLError(
          "Users outside of the admin group cannot approve new users.",
          {
            extensions: { code: "404" }
          }
        );
    ...
    updateEvalData: async (obj, args, context, inflow) => {
      ...
      if (session?.valid && session?.userId == user?._id && user?.admin) {
        ...
        throw new GraphQLError(
          "Users outside of the admin group cannot update evals.",
          {
            extensions: { code: "404" }
          }
        );
    ...
    addNewEval: async (obj, args, context, inflow) => {
      ...
      if (session?.valid && session?.userId == user?._id && user?.admin) {
        ...
        throw new GraphQLError(
          "Users outside of the admin group cannot add evals.",
          {
            extensions: { code: "404" }
          }
        );
    ...
    deleteEval: async (obj, args, context, inflow) => {
      ...
      if (session?.valid && session?.userId == user?._id && user?.admin) {
        try {
          const result = await context.db
            .collection("evalData")
            .deleteOne({ _id: ObjectId(args["evalId"]) });
          return result;
        } catch (error) {
        ...
        throw new GraphQLError(
          "Users outside of the admin group cannot delete evals.",
          {
            extensions: { code: "404" }
          }
        );
    ...
    uploadScenarioResults: async (obj, args, context, inflow) => {
      ...
      for (const result of results) {
        if (result.participantID.toLowerCase().includes("test")) {
          continue;
        }
    ...
    addNewParticipantToLog: async (obj, args, context, inflow) => {
      try {
        const pidBounds = await context.db
          .collection("surveyVersion")
          .findOne();
        ...
        if (!Number.isFinite(args.participantData.ParticipantID)) {
          console.log(
            `[${timestamp}] Invalid PID detected, querying for highest PID`
          );

          const highestPidDoc = await context.db
            .collection("participantLog")
            .find({
          ...
          generatedPid =
            highestPidDoc.length > 0
              ? Number(highestPidDoc[0].ParticipantID) + 1
              : lowPid;
        ...
        try {
          ...
          console.log(
            `[${timestamp}] Insert SUCCESS for PID: ${args.participantData.ParticipantID}`
          );
          return { ...result, generatedPid };
        } catch (error) {
          if (error.code === 11000) {
            // ff we hit a duplicate
            console.log(
              `[${timestamp}] DUPLICATE KEY ERROR for PID: ${args.participantData.ParticipantID}`
            );
            ...
            console.log(
              `[${timestamp}] Attempting final insert with PID: ${retryPid}`
            );
            const retryResult = await context.db
              .collection("participantLog")
              .insertOne(args.participantData);
            console.log(
              `[${timestamp}] Retry insert SUCCESS for PID: ${retryPid}`
            );
            return retryResult;
          }
        ...
        const timestamp = generateServerTimestamp();
        console.error(
          `[${timestamp}] CRITICAL ERROR in addNewParticipantToLog:`,
          error
        );
        if (error.code === 11000) {
          return -1;
    ...
    deleteDataByPID: async (obj, args, context, inflow) => {
      ...
      const user = await context.db
        .collection("users")
        .find({ username: args["caller"]?.["user"]?.["username"] })
        ?.project({ _id: 1, username: 1, admin: 1 })
        .toArray()
        .then((result) => {
          return result[0];
      ...
      if (session?.valid && session?.userId == user?._id && user?.admin) {
        const foundPlog = await context.db
          .collection("participantLog")
          .find({ ParticipantID: Number(args["pid"]) })
          .toArray();
        const foundText = await context.db
          .collection("userScenarioResults")
          .find({ participantID: args["pid"] })
          .toArray();
        const foundSurvey = await context.db
          .collection("surveyResults")
          .find({
            "results.Participant ID Page.questions.Participant ID.response":
              args["pid"]
        ...
        if (!allOldEnough) {
          throw new GraphQLError(
            "Data is not yet 24 hours old. Please wait to delete.",
            {
              extensions: { code: "400" }
            }
          );
        ...
        const plogRes = await context.db
          .collection("participantLog")
          .deleteMany({ ParticipantID: Number(args["pid"]) });
        const textRes = await context.db
          .collection("userScenarioResults")
          .deleteMany({ participantID: args["pid"] });
        const surveyRes = await context.db
          .collection("surveyResults")
          .deleteMany({
            "results.Participant ID Page.questions.Participant ID.response":
              args["pid"]
          });
        const rawSimRes = await context.db
          .collection("humanSimulatorRaw")
          .deleteMany({ pid: args["pid"] });
        const analyzedSimRes = await context.db
          .collection("humanSimulator")
          .deleteMany({ pid: args["pid"] });
        return { plogRes, textRes, surveyRes, rawSimRes, analyzedSimRes };
      } else {
        throw new GraphQLError(
          "Users outside of the admin group cannot delete participant data.",
          {
            extensions: { code: "404" }
          }
        );
